...
[(Inferenzstatistische) [*Schätzung*]{.imp} (estimation with quantified uncertainty) treffen anhand von Stichproben Aussagen über Parameter der Grundgesamtheit (Population) aus der die Stichprobe gezogen wurde.  
(Inferenztsatistische) [*Hypothesentests*]{.imp} bewerten anhand von Stichprobendaten die Gültigkeit von Hypothesen in der Grundgesamtheit (Population) aus der die Stichprobe gezogen wurde.]{.fragment fragment-index=7}


## Bsp.: Parameterschätzung {.smaller}
> Forschungsfrage: Welcher Anteil der Eltern befürwortet eine Grundschule ohne Noten?
...
ggplot(dat_long) + 
  ...
  labs(title = "Posterior Estimation", 
       ...
       caption = paste("Der »Vertrauenbereich« (95% Highest Densitiy Interval)\nreicht von ",
...
* Hypothesen 
    ...
    * Alternativhypothese: *Mindestens 50% der Eltern befürworten eine Grundschule mit Noten*
* Daten: 
    * 3 für »mit Noten«
    * 1 für »ohne Noten«
:::
...
::: {.callout-tip icon=false collapse="false"}
## {{< fa chart-column >}} Ergebnis
Ein Vergleich der beiden Gruppen einem Posttest ergibt ein Cohen's $U_3 = .53$ mit einem *p*-Wert von .073 für die Nullhypothese $H_0:\; U_3 = .5$ und die Alternativhypothese $H_A:\; U_3 > .5$.
:::
...
::: {.callout-tip icon=false collapse="false"}
## {{< fa chart-column >}} Ergebnis
Ein Vergleich der beiden Gruppen ergibt ein Cliff's d von .04 mit einem *p*-Wert von .008 für die Nullhypothese $H_0:\; d = 0$ und die Alternativhypothese $H_0:\; d > 0$
:::
...
::: {.callout-tip icon=false collapse="false"}
## {{< fa lightbulb >}} Interpretation
In den Daten liegt ein Effekt sehr geringer Größe vor. Dennoch $H_0$ kann zugunsten von $H_A$ verworfen werden.
:::
...
## Bayes-Faktoren: Definition {.smaller}
...
## Testung mit Bayes-Faktor: Beispiel 2
::: {.callout-tip icon=false collapse="false"}
## {{< fa question-circle >}} Forschungsfrage & -design
Forschende versuchen den Lernstilmythos zu widerlegen. Dazu befragen Sie Probandinnen nach ihrem vermeintlichen Lernstil. Diese durchlaufen danach eine Unterrichtseinheit zum Thema Photosynthese entweder (randomisiert) gemäß dieses vermeintlichen Lernstils oder nicht gemäß diesem.
:::
...
::: {.callout-tip icon=false collapse="false"}
## {{< fa chart-column >}} Ergebnis
Ein Vergleich der beiden Gruppen einem Posttest ergibt ein Cohen's $U_3 = .53$ mit einem $BF{10}=.001 = \frac{1}{1000}$ für die Nullhypothese $H_0:\; U_3 = .5$ und die Alternativhypothese $H_A:\; U_3 > .5$.
:::
